...
pub const ADSR = struct {
    ...
    pub fn process(ptr: *anyopaque) void {
        var adsr: *ADSR = @ptrCast(@alignCast(ptr));
        ...
        if (adsr.hold_duration >= ramp_duration) {
            const next_state: ADSR.State = @enumFromInt(@intFromEnum(adsr.state) + 1);
    ...
    pub fn node(ptr: *ADSR) signals.Node {
        return signals.Node.init(ptr, ADSR);
    }
};
